...
%Options list
%Options fp=Modula2pim4Lexer
%options single_productions
%options conflicts
%options package=lpg.grammars.modula2pim4
%options template=LexerTemplateF.gi
%options filter=Modula2pim4KWLexer.gi

%Define
    $kw_lexer_class /.$Modula2pim4KWLexer./
%End
...
%Rules
    Token ::= STRING /. makeToken($_STRING); ./
            ...
            | SLComment     /. skipToken(); ./ | MLComment     /. skipToken(); ./ | PSComment     /. skipToken(); ./
            | white /. skipToken(); ./
    ...
    PSComment ::= '(' '*' PSCBody '*' ')'
    PSCBody -> $empty | PSCBody NotStarP | PSCBody '*' NotRParen
    NotStarP -> Letter | Digit | SpecialNotStar | Space | HT | FF | LF | CR | AfterASCII | '_' | '(' | ')' | "'" | '"'
    NotRParen -> Letter | Digit | SpecialNotSlash | Space | HT | FF | LF | CR | AfterASCII | '_' | '*' | '(' | "'" | '"'
