-- Keyword filter for SqlMariadb (from grammars-v4 lexer keywords + parser literals)
%options package=lpg.grammars.sql.mariadb
%options template=KeywordTemplateF.gi
%options fp=SqlMariadbKWLexer
...
%Rules
    ...
    Keyword ::= p e r s i s t e n t /.$setResult($_PERSISTENT);./
    ...
    Keyword ::= r e p l i c a s /.$setResult($_REPLICAS);./
    ...
    Keyword ::= l a s t v a l /.$setResult($_LASTVAL);./
    Keyword ::= l a t e r a l /.$setResult($_LATERAL);./
    Keyword ::= l e a d i n g /.$setResult($_LEADING);./
    Keyword ::= l o c a l e s /.$setResult($_LOCALES);./
    ...
    Keyword ::= r e p l a y /.$setResult($_REPLAY);./
    ...
    Keyword ::= s e t v a l /.$setResult($_SETVAL);./
    ...
    Keyword ::= s l a v e s /.$setResult($_SLAVES);./
