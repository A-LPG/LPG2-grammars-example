...
%Rules
    ...
    list_1 ::= $empty | list_1 external_declaration

    opt_2 ::= function_call_parameters | $empty

    opt_3 ::= function_call_parameters | $empty

    opt_4 ::= function_call_parameters | $empty

    seq_5 ::= COMMA assignment_expression

    list_6 ::= $empty | list_6 seq_5

    grp_7 ::= STAR | SLASH | PERCENT

    grp_8 ::= PLUS | MINUS

    grp_9 ::= LSHIFT | RSHIFT

    grp_10 ::= LEFT_ANGLE | RIGHT_ANGLE | LTEQ | GTEQ

    grp_11 ::= EQEQ | NOTEQ

    opt_12 ::= array_specifier | $empty

    seq_13 ::= IDENTIFIER opt_12

    opt_14 ::= seq_13 | $empty

    opt_15 ::= identifier_list | $empty

    seq_16 ::= COMMA IDENTIFIER

    list_17 ::= $empty | list_17 seq_16

    opt_18 ::= function_parameters | $empty

    seq_19 ::= COMMA parameter_declaration

    list_20 ::= $empty | list_20 seq_19

    opt_21 ::= array_specifier | $empty

    grp_22 ::= parameter_declarator | parameter_type_specifier

    seq_23 ::= COMMA typeless_declaration

    list_24 ::= $empty | list_24 seq_23

    opt_25 ::= typeless_declaration | $empty

    opt_26 ::= array_specifier | $empty
    ...
    list_33 ::= single_type_qualifier | list_33 single_type_qualifier
    ...
    opt_38 ::= array_specifier | $empty

    list_39 ::= dimension | list_39 dimension

    opt_40 ::= constant_expression | $empty

    opt_41 ::= IDENTIFIER | $empty

    list_42 ::= struct_declaration | list_42 struct_declaration

    seq_43 ::= COMMA struct_declarator

    list_44 ::= $empty | list_44 seq_43

    opt_45 ::= array_specifier | $empty
    ...
    opt_49 ::= statement_list | $empty

    opt_50 ::= statement_list | $empty

    list_51 ::= statement | list_51 statement

    seq_52 ::= ELSE statement

    opt_53 ::= seq_52 | $empty

    opt_54 ::= statement_list | $empty

    opt_55 ::= condition | $empty

    opt_56 ::= expression | $empty

    opt_57 ::= expression | $empty
