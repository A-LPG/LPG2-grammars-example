...
%Rules
    ...
    list_1 ::= tuple | list_1 tuple

    opt_2 ::= REFUSE | $empty

    grp_3 ::= sizing | crossover | qualifier

    list_4 ::= $empty | list_4 grp_3

    seq_5 ::= LPAREN sizing RPAREN

    grp_6 ::= MINUS | PLUS | STAR

    list_7 ::= grp_6 | list_7 grp_6

    grp_8 ::= BSD | U | AIX | SYSV | HPUX | IRIX | OSF1 | SOLARIS | SCO | NEXT | LINUX

    list_9 ::= grp_8 | list_9 grp_8

    opt_10 ::= sizing | $empty

    list_11 ::= $empty | list_11 qualifier

    opt_12 ::= sizing | $empty

    list_13 ::= $empty | list_13 qualifier
