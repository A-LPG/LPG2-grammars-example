...
%Rules
    ...
    grp_1 ::= element | empty_line

    list_2 ::= grp_1 | list_2 grp_1

    list_3 ::= $empty | list_3 Space

    list_4 ::= $empty | list_4 Space

    opt_5 ::= commentParagraphs | $empty
    ...
    list_10 ::= commentLine | list_10 commentLine

    list_11 ::= LineBreak | list_11 LineBreak

    seq_12 ::= LineBreak SectionSeparator

    opt_13 ::= seq_12 | $empty

    opt_14 ::= LineBreak | $empty

    list_15 ::= $empty | list_15 LineBreak

    list_16 ::= $empty | list_16 sectionElement

    list_17 ::= Space | list_17 Space

    opt_18 ::= paragraphNoBreak | $empty

    opt_19 ::= LineBreak | $empty

    list_20 ::= $empty | list_20 lineBlockLine

    opt_21 ::= indentation | $empty

    list_22 ::= $empty | list_22 span

    opt_23 ::= indentation | $empty

    list_24 ::= span | list_24 span

    list_25 ::= $empty | list_25 Space

    grp_26 ::= Minus | Plus

    list_27 ::= $empty | list_27 Space

    list_28 ::= $empty | list_28 Space

    list_29 ::= paragraph | list_29 paragraph

    opt_30 ::= list_29 | $empty

    list_31 ::= $empty | list_31 Space

    list_32 ::= Space | list_32 Space

    list_33 ::= $empty | list_33 paragraph

    list_34 ::= Space | list_34 Space

    list_35 ::= $empty | list_35 paragraph

    seq_36 ::= paragraphNoBreak list_35

    opt_37 ::= seq_36 | $empty

    list_38 ::= $empty | list_38 lines

    opt_39 ::= indentation | $empty

    list_40 ::= $empty | list_40 span

    opt_42 ::= linesNormal | $empty

    grp_41 ::= linesStar | opt_42

    opt_43 ::= linesNormal | $empty

    opt_44 ::= indentation | $empty

    list_45 ::= spanLineStartNoStar | list_45 spanLineStartNoStar

    list_46 ::= $empty | list_46 span

    list_47 ::= spanNoStar | list_47 spanNoStar

    seq_48 ::= list_46 list_47

    opt_49 ::= seq_48 | $empty

    opt_50 ::= indentation | $empty

    list_51 ::= $empty | list_51 spanLineStartNoStar

    opt_52 ::= indentation | $empty

    list_53 ::= text_fragment | list_53 text_fragment

    opt_54 ::= indentation | $empty

    opt_55 ::= indentation | $empty

    list_56 ::= Space | list_56 Space

    seq_57 ::= list_56 SectionSeparator

    list_58 ::= $empty | list_58 Space

    list_59 ::= $empty | list_59 Space

    list_60 ::= Space | list_60 Space

    list_61 ::= lineStart_fragment | list_61 lineStart_fragment

    list_62 ::= $empty | list_62 text_fragment

    grp_64 ::= Space | LineBreak | Star
    ...
    grp_68 ::= Space | LineBreak | Star

    grp_67 ::= Numbers Dot grp_68

    grp_70 ::= Dot | LineBreak | Star
    ...
    grp_77 ::= Dot | LineBreak | Star

    grp_76 ::= Alphabet grp_77

    list_78 ::= textStart | list_78 textStart

    list_79 ::= $empty | list_79 text_fragment

    list_80 ::= text_fragment_start | list_80 text_fragment_start

    list_81 ::= Star | list_81 Star

    list_82 ::= Star | list_82 Star

    list_83 ::= $empty | list_83 Star

    seq_84 ::= LineBreak list_83 starNoSpace starAtoms

    list_85 ::= $empty | list_85 seq_84

    list_86 ::= $empty | list_86 Star

    list_87 ::= Star | list_87 Star

    list_88 ::= $empty | list_88 Star

    list_89 ::= Star | list_89 Star

    list_90 ::= Space | list_90 Space

    list_91 ::= Star | list_91 Star

    list_92 ::= $empty | list_92 starAtom

    list_93 ::= $empty | list_93 Star

    seq_94 ::= list_93 starAtom

    list_95 ::= $empty | list_95 seq_94

    grp_96 ::= Star | LineBreak | Space | SectionSeparator

    grp_97 ::= Star | LineBreak

    seq_98 ::= Colon Alphabet Colon

    opt_99 ::= seq_98 | $empty

    opt_100 ::= UnderScore | $empty

    list_101 ::= $empty | list_101 BackTick

    list_102 ::= BackTick | list_102 BackTick

    list_103 ::= BackTick | list_103 BackTick

    list_104 ::= backTickAtom | list_104 backTickAtom

    grp_105 ::= BackTick | LineBreak | Space

    grp_106 ::= BackTick | LineBreak

    grp_107 ::= BackTick | LineBreak

    list_108 ::= Any | list_108 Any

    list_109 ::= hyperlinkAtom | list_109 hyperlinkAtom

    list_110 ::= Any | list_110 Any

    list_111 ::= hyperlinkAtom | list_111 hyperlinkAtom

    list_112 ::= hyperlinkAtom | list_112 hyperlinkAtom

    list_113 ::= urlAtom | list_113 urlAtom

    grp_114 ::= LineBreak | BackTick

    grp_115 ::= LineBreak | AngleLeft | AngleRight | BackTick | Star

    grp_116 ::= Minus | Equal | Plus | Hat
