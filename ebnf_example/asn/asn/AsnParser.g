...
%Rules
    ...
    grp_32 ::= optionalExtensionMarker
              | ELLIPSIS COMMA rootComponentTypeList

    grp_31 ::= COMMA extensionAndException extensionAdditions grp_32

    opt_33 ::= grp_31?

    grp_34 ::= optionalExtensionMarker
              | ELLIPSIS COMMA rootComponentTypeList

    seq_35 ::= COMMA componentType

    listn36 ::= (listn36 seq_35)?

    grp_37 ::= OPTIONAL_LITERAL
              | DEFAULT_LITERAL value
    ...
    optn59 ::= withSyntaxSpec?

    list_60 ::= tokenOrGroupSpec+

    list_61 ::= tokenOrGroupSpec+

    opt_63 ::= typeOptionalitySpec?

    opt_65 ::= valueSetOptionalitySpec?

    opt_66 ::= UNIQUE_LITERAL?

    opt_67 ::= valueOptionalitySpec?
    ...
    grpn68 ::= OPTIONAL_LITERAL
              | DEFAULT_LITERAL grpn69
    ...
    grpn71 ::= OPTIONAL_LITERAL
              | DEFAULT_LITERAL grpn72
    ...
    grpn62 ::= opt_63
              | asnType grpn64
              | fieldName opt_70
              | definedObjectClass opt_73

    opt_74 ::= typeOptionalitySpec?

    opt_75 ::= UNIQUE_LITERAL?

    opt_76 ::= valueOptionalitySpec?

    opt_77 ::= valueOptionalitySpec?

    opt_78 ::= valueSetOptionalitySpec?
    ...
    list_93 ::= (list_93 seqn92)?

    seqn94 ::= intersectionMark intersectionElements

    list_95 ::= (list_95 seqn94)?
    ...
    optn101 ::= valueSetOptionalitySpec?

    optn102 ::= objectOptionalitySpec?

    optn103 ::= objectSetOptionalitySpec?

    grp_104 ::= builtinType
               | referencedType

    listn105 ::= (listn105 constraint)?

    grp_106 ::= extensionAndException optionalExtensionMarker
               | componentTypeLists
    ...
    opt_113 ::= actualParameterList?

    opt_114 ::= exceptionSpec?

    seqn115 ::= COMMA userDefinedConstraintParameter

    listn116 ::= (listn116 seqn115)?
    ...
    listn127 ::= (listn127 seq_126)?

    grp_128 ::= ABSENT_LITERAL
               | PRESENT_LITERAL

    listn129 ::= (listn129 objIdComponents)?
    ...
    listn139 ::= (listn139 seq_138)?

    seq_140 ::= COMMA namedType

    listn141 ::= (listn141 seq_140)?

    optn142 ::= exceptionSpec?
    ...
    listn148 ::= (listn148 seq_147)?

    grp_149 ::= signedNumber
               | definedValue

    optn150 ::= actualParameterList?
