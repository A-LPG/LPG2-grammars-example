...
%Rules
    ...
    grp_20 ::= STAR opt_21 list_25 opt_30
              | STARSTAR tfpdef opt_31
    ...
    grp_12 ::= tfpdef opt_14 list_18 opt_33
              | STAR opt_34 list_38 opt_43
              | STARSTAR tfpdef opt_44
    ...
    grp_55 ::= STAR opt_56 list_60 opt_65
              | STARSTAR vfpdef opt_66
    ...
    grp_47 ::= vfpdef opt_49 list_53 opt_68
              | STAR opt_69 list_73 opt_78
              | STARSTAR vfpdef opt_79
    ...
    list_121 ::= (list_121 seq_120)?

    seq_122 ::= COMMA name

    list_123 ::= (list_123 seq_122)?
    ...
    grp_136 ::= list_138 opt_140 opt_142
               | FINALLY COLON block
    ...
    opt_153 ::= star_named_expressions?

    list_154 ::= star_named_expression+
    ...
    list_158 ::= (list_158 grp_157)?

    opt_159 ::= signed_number?

    opt_160 ::= signed_number?
    ...
    opt_165 ::= maybe_sequence_pattern?

    opt_166 ::= open_sequence_pattern?

    opt_167 ::= maybe_sequence_pattern?

    seq_168 ::= COMMA maybe_star_pattern

    list_169 ::= (list_169 seq_168)?
    ...
    list_175 ::= (list_175 seq_174)?

    grp_176 ::= literal_expr
               | attr
    ...
    list_181 ::= (list_181 seq_180)?

    seq_182 ::= COMMA keyword_pattern

    list_183 ::= (list_183 seq_182)?
    ...
    list_189 ::= (list_189 seq_188)?

    seq_190 ::= AND not_test

    list_191 ::= (list_191 seq_190)?

    seq_192 ::= comp_op expr

    list_193 ::= (list_193 seq_192)?
    ...
    opt_203 ::= testlist_comp?

    opt_204 ::= dictorsetmaker?
    ...
    list_223 ::= (list_223 grp_221)?

    opt_224 ::= COMMA?

    seq_225 ::= COMMA test

    list_226 ::= (list_226 seq_225)?

    opt_227 ::= COMMA?

    grp_230 ::= test COLON test
               | STARSTAR expr

    grp_233 ::= test COLON test
               | STARSTAR expr

    grp_232 ::= COMMA grp_233

    list_234 ::= (list_234 grp_232)?
    ...
    list_247 ::= (list_247 seq_246)?

    opt_248 ::= COMMA?

    opt_250 ::= comp_for?

    grp_249 ::= test opt_250
               | test EQ test
               | STARSTAR test
               | STAR test
