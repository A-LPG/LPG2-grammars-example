-- Keyword filter for XpathXpath20 (from grammars-v4 lexer keywords + parser literals)
...
%Rules
    Keyword ::= p r o c e s s i n g i n s t r u c t i o n /.$setResult($_KW_PROCESSING_INSTRUCTION);./
    Keyword ::= d e s c e n d a n t o r s e l f /.$setResult($_KW_DESCENDANT_OR_SELF);./
    Keyword ::= f o l l o w i n g s i b l i n g /.$setResult($_KW_FOLLOWING_SIBLING);./
    Keyword ::= p r e c e d i n g s i b l i n g /.$setResult($_KW_PRECEDING_SIBLING);./
    Keyword ::= s c h e m a a t t r i b u t e /.$setResult($_KW_SCHEMA_ATTRIBUTE);./
    Keyword ::= a n c e s t o r o r s e l f /.$setResult($_KW_ANCESTOR_OR_SELF);./
    Keyword ::= e m p t y s e q u e n c e /.$setResult($_KW_EMPTY_SEQUENCE);./
    Keyword ::= n a m e s p a c e n o d e /.$setResult($_KW_NAMESPACE_NODE);./
    Keyword ::= s c h e m a e l e m e n t /.$setResult($_KW_SCHEMA_ELEMENT);./
    Keyword ::= d o c u m e n t n o d e /.$setResult($_KW_DOCUMENT_NODE);./
