...
%Rules
    ...
    -- Same address/CONNECT_DATA factoring as description (single-alias entries
    -- often reduce via lsnr_entry rather than tns_entry).
    lsnr_description ::= LPAREN DESCRIPTION EQ lsnr_desc_body RPAREN

    lsnr_desc_body ::= address_list
           | address desc_more
    ...
    -- LPAREN-factored: after an address, ADDRESS vs CONNECT_DATA vs RPAREN
    -- must not be a shift/reduce that always prefers another address.
    description ::= LPAREN DESCRIPTION EQ opt_16 desc_body RPAREN

    desc_body ::= address_list opt_19 opt_connect opt_20
           | address desc_more

    desc_more ::= LPAREN desc_after_lparen
           | $empty

    desc_after_lparen ::= ADDRESS EQ protocol_info opt_29 RPAREN desc_more
           | CONNECT_DATA EQ cd_params RPAREN

    opt_connect ::= connect_data
           | $empty
