...
%Rules
    ...
    opt_params ::= $empty | opt_params COLON name
    opt_eol ::= $empty | EOL
    ...
    comparisonOperator ::= LT | GT | EQUAL
    ...
    atom ::= number | deref | func_
    number ::= NUMBER
    func_ ::= random
    random ::= RANDOM expression

    fd ::= FD expression | FORWARD expression
    bk ::= BK expression | BACKWARD expression
    rt ::= RT expression | RIGHT expression
    lt ::= LT expression | LEFT expression
    cs ::= CS | CLEARSCREEN
    pu ::= PU | PENUP
    pd ::= PD | PENDOWN
    ht ::= HT | HIDETURTLE
