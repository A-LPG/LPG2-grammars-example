...
%Rules
    ...
    opt_1 ::= literal_attribs | $empty

    opt_2 ::= section_members | $empty

    opt_3 ::= section_members | $empty

    opt_4 ::= literal_attribs | $empty

    opt_5 ::= SHARED | $empty

    opt_6 ::= NULLABLE | $empty

    opt_7 ::= argument_list | $empty

    opt_8 ::= item_list | $empty

    opt_9 ::= field_list | $empty

    opt_10 ::= parameter_list | $empty

    opt_11 ::= return_type | $empty

    opt_12 ::= parameter_type | $empty

    opt_13 ::= field_specification_list | $empty

    opt_14 ::= OPTIONAL_TEXT | $empty

    opt_15 ::= field_type_specification | $empty

    opt_16 ::= parameter_specification_list | $empty

    opt_17 ::= otherwise_clause | $empty

    opt_18 ::= literal_field_list | $empty

    opt_19 ::= literal_item_list | $empty
