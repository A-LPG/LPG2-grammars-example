...
%Rules
    ...
    class_declaration ::= opt_185 class_header opt_186 opt_187 opt_188 opt_189 opt_190 opt_191 opt_192 opt_193 opt_194 END

    notes ::= NOTE note_list

    note_list ::= opt_199 list_200

    note_entry ::= note_name note_values

    note_name ::= Identifier COLON

    note_values ::= note_item list_202 opt_203

    note_item ::= Identifier
           | manifest_constant

    class_header ::= list_204 CLASS class_name
    ...
    feature_clause ::= FEATURE opt_206 feature_declaration_list

    feature_declaration_list ::= opt_211 list_212

    feature_declaration ::= new_feature_list declaration_body

    declaration_body ::= opt_213 opt_214 opt_215 feature_value

    query_mark ::= type_mark opt_216

    type_mark ::= COLON type

    feature_value ::= opt_217 opt_218 opt_219 opt_221

    explicit_value ::= EQ manifest_constant

    new_feature_list ::= new_feature list_223

    new_feature ::= opt_224 extended_feature_name

    attribute_or_routine ::= opt_225 opt_226 feature_body opt_227 opt_228 END

    feature_body ::= DEFERRED
           | effective_routine
           | attribute

    extended_feature_name ::= feature_name list_229

    feature_name ::= Identifier

    alias ::= ALIAS manifest_string opt_230

    assigner_mark ::= ASSIGN feature_name

    inheritance ::= list_231

    inherit_clause ::= INHERIT opt_232 parent_list

    non_conformance ::= LBRACE Identifier RBRACE

    parent_list ::= parent list_235 list_236

    parent ::= class_type opt_237

    feature_adaptation ::= list_239 END

    rename ::= RENAME rename_list

    rename_list ::= rename_pair list_241

    rename_pair ::= feature_name AS extended_feature_name

    clients ::= LBRACE opt_242 RBRACE

    class_list ::= class_name list_244

    new_exports ::= EXPORT new_export_list

    new_export_list ::= new_export_item list_247 list_248
    ...
    feature_list ::= opt_252

    formal_arguments ::= LPAREN entity_declaration_list RPAREN

    entity_declaration_list ::= entity_declaration_group list_255 list_256

    entity_declaration_group ::= identifier_list type_mark

    identifier_list ::= Identifier list_258
    ...
    key_list ::= manifest_string list_262

    local_declarations ::= LOCAL opt_263

    compound ::= list_264 opt_269 list_270
    ...
    precondition ::= REQUIRE opt_271 assertion

    postcondition ::= ENSURE opt_272 assertion opt_273

    invariant ::= INVARIANT assertion

    assertion ::= opt_278 list_279

    assertion_clause ::= opt_280 unlabeled_assertion_clause
           | tag_mark
    ...
    check ::= CHECK assertion opt_281 END

    variant ::= VARIANT opt_282 expression

    precursor ::= PRECURSOR opt_283 opt_284
    ...
    class_type ::= opt_287 class_name opt_288

    attachment_mark ::= QUESTION
           | BANG

    anchored ::= opt_289 LIKE anchor
    ...
    formal_generic_list ::= formal_generic list_295

    formal_generic ::= list_296 formal_generic_name opt_297

    formal_generic_modifier ::= FROZEN
           | REFERENCE

    formal_generic_name ::= opt_298 Identifier

    constraint ::= ARROW constraining_types opt_299

    constraining_types ::= single_constraint
           | multiple_constraint

    single_constraint ::= type opt_300

    renaming ::= rename END

    multiple_constraint ::= LBRACE constraint_list RBRACE

    constraint_list ::= single_constraint list_302

    constraint_creators ::= CREATE feature_list END

    manifest_array ::= opt_303 LSHIFT expression_list RSHIFT

    manifest_array_type ::= LBRACE type RBRACE

    expression_list ::= opt_307
    ...
    converter_list ::= converter list_309
    ...
    conditional ::= IF then_part_list opt_310 END

    then_part_list ::= then_part list_312
    ...
    then_part_expression_list ::= then_part_expression list_314

    then_part_expression ::= expression THEN expression

    multi_branch ::= INSPECT expression opt_315 opt_316 END

    when_part_list ::= list_317

    when_part ::= WHEN choices THEN compound

    choices ::= choice list_319
    ...
    loop ::= opt_320 opt_321 opt_322 opt_323 opt_324 loop_body opt_325 END
    ...
    debug ::= DEBUG opt_327 compound END
    ...
    creation_clause ::= CREATE opt_329 opt_330

    creation_procedure_list ::= creation_procedure list_332

    creation_procedure ::= feature_name

    creation_instruction ::= CREATE opt_333 opt_334 creation_call

    create_passive_region ::= LT Identifier GT

    explicit_creation_type ::= LBRACE type RBRACE

    creation_call ::= variable opt_335

    explicit_creation_call ::= DOT unqualified_call

    creation_expression ::= CREATE explicit_creation_type opt_336

    assignment ::= variable COLONEQ expression

    assigner_call ::= expression COLONEQ expression

    unqualified_call ::= feature_name opt_337

    non_object_call ::= LBRACE type RBRACE DOT unqualified_call

    actuals ::= LPAREN actual_list RPAREN

    actual_list ::= expression list_339
    ...
    inline_separate_arguments ::= inline_separate_argument list_341
    ...
    inline_agent ::= AGENT opt_342 opt_343 opt_344 opt_345
    ...
    agent_unqualified ::= feature_name opt_346
    ...
    agent_actual_list ::= agent_actual list_348

    agent_actual ::= expression
           | placeholder

    placeholder ::= opt_349 QUESTION

    expression ::= expression DOT expression
           | grp_350 expression
           | expression FreeOperator expression
           | expression CARET expression
           | expression grp_351 expression
           | expression grp_352 expression
           | expression DOTDOT expression
           | expression grp_353 expression
           | expression grp_354 expression
           | expression grp_355 expression
           | expression IMPLIES expression
           | expression LBRACKET expression_list RBRACKET
           | expression LBRACKET actuals RBRACKET
           | LPAREN expression RPAREN
           | OP_59361 expression OP_24890
           | grp_356 Identifier COLON expression BAR expression
           | Identifier
           | unqualified_call
           | CURRENT
           | RESULT
           | VOID
           | ATTACHED opt_358 expression opt_360
           | ACROSS expression AS Identifier grp_361 expression END
           | ONCE opt_363 expression
           | precursor
           | creation_expression
           | conditional_expression
           | non_object_call
    ...
    manifest_constant ::= opt_365 manifest_value
    ...
    integer_constant ::= opt_366 Integer

    boolean_constant ::= TRUE
           | FALSE

    real_constant ::= opt_367 Real

    manifest_string ::= Basic_manifest_string
           | Verbatim_string

    external ::= EXTERNAL external_language opt_368
